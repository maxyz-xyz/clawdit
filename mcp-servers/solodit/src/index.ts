...
function ensureApiKey(): void {
  if (!API_KEY) {
    throw new Error(
      "SOLODIT_API_KEY not set. Get your key at: https://solodit.cyfrin.io → Profile → API Keys, then export SOLODIT_API_KEY=sk_your_key",
    );
  }
}
...
server.tool(
  "search_findings",
  "Search Solodit's 20k+ smart contract security findings from real audits. Returns severity, firm, protocol, tags, quality score, content snippet, and Solodit URL.",
  {
    keywords: z
      .string()
      .optional()
  ...
  async (params) => {
    ...
    if (params.firms) filters.firms = params.firms.map((v) => ({ value: v }));
    if (params.tags) filters.tags = params.tags.map((v) => ({ value: v }));
    if (params.language) filters.languages = [{ value: params.language }];
    if (params.protocol) filters.protocol = params.protocol;
    if (params.reported) filters.reported = { value: params.reported };
    if (params.sort_by) {
    ...
    if (adv) {
      ...
      if (adv.forked) filters.forked = adv.forked.map((v) => ({ value: v }));
    }
...
server.tool(
  "get_finding",
  "Get full details for a specific Solodit finding by numeric ID, URL, or slug.",
  {
    identifier: z
      .string()
  ...
  async (params) => {
    ...
    let finding = data.findings.find(
      (f) => f.slug === slug || f.id === numericId,
    );
...
server.tool(
  "get_filter_options",
  "List available filter values for Solodit search (firms, tags, categories, languages).",
  {},
  async () => {
