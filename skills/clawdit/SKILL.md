...
2. **Resolve references**: Use Glob for `${CLAUDE_PLUGIN_ROOT}/skills/clawdit/references/protocol-detection.md` to confirm the references directory path. The parent directory of this file is `{ref}` — use it for all subsequent reads (e.g., `{ref}/methodology/judging.md`, `{ref}/protocols/lending.md`).
...
4. Create four per-agent bundle files via a single Bash command. All reference files use the resolved `{ref}` path (the absolute path to `${CLAUDE_PLUGIN_ROOT}/skills/clawdit/references`). The command must:

```bash
# For each agent N (1-4), concatenate these files into /tmp/audit-agent-N-bundle.md:
